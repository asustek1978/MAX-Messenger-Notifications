...
PERMISSION_OPTIONS = [
    {"value": PERM_ALL, "label": "Полный доступ"},
    {"value": "notifications", "label": "Получать уведомления"},
    {"value": "lights", "label": "Освещение"},
    {"value": "climate", "label": "Климат"},
    {"value": "devices", "label": "Устройства"},
    {"value": "water", "label": "Краны воды"},
    {"value": "vacuum", "label": "Робот-пылесос Масяня"},
    {"value": "braga", "label": "Braga Controller"},
    {"value": "braga_emergency", "label": "Braga: аварийная остановка"},
    {"value": "status", "label": "Состояние дома"},
    {"value": "intercom", "label": "Домофон"},
    {"value": "intercom_open", "label": "Домофон: открытие двери"},
    {"value": "cameras", "label": "Камеры"},
    {"value": "scenes", "label": "Сценарии дома"},
]
PERMISSIONS_SELECTOR = selector.SelectSelector(
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(
                        CONF_TARGET_TYPE, default=TARGET_USER
                    ): vol.In([TARGET_USER, TARGET_CHAT]),
...
class SlavaMaxOptionsFlow(OptionsFlowWithReload):
    ...
    def _mobile_notify_options(self, current: str = "") -> list[dict[str, str]]:
        ...
        if current and current.startswith("notify.mobile_app_") and current not in values:
    ...
    async def async_step_init(
        ...
        return self.async_show_menu(
            step_id="init",
            menu_options=["general", "fallback", "users"],
        )
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_TARGET_TYPE,
                        default=current.get(CONF_TARGET_TYPE, TARGET_USER),
                    ): vol.In([TARGET_USER, TARGET_CHAT]),
                    vol.Required(
                        CONF_TARGET_ID,
                        default=current.get(CONF_TARGET_ID, 0),
                    ): int,
                    vol.Required(
                        CONF_POLLING,
    ...
    async def async_step_fallback(
        ...
        current = self._current()
        if user_input is not None:
            return self._save_options(dict(user_input))

        current_ha_service = str(current.get(CONF_FALLBACK_HA_SERVICE, "") or "")
        mobile_options = self._mobile_notify_options(current_ha_service)

        if mobile_options:
            mobile_selector = selector.SelectSelector(
                selector.SelectSelectorConfig(
            ...
            mobile_selector = selector.TextSelector()
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_FALLBACK_HA_ENABLED,
                        default=current.get(CONF_FALLBACK_HA_ENABLED, False),
                    ): bool,
                    vol.Optional(
                        CONF_FALLBACK_HA_SERVICE,
                        default=current_ha_service,
                    ): mobile_selector,
                    vol.Required(
                        CONF_FALLBACK_VK_ENABLED,
                        default=current.get(CONF_FALLBACK_VK_ENABLED, False),
                    ): bool,
                    vol.Optional(
                        CONF_FALLBACK_VK_ENTITY,
                        default=str(
                            current.get(CONF_FALLBACK_VK_ENTITY, "notify.poliarnaia")
                    ...
                    vol.Required(
                        CONF_FALLBACK_REGULAR,
                        default=current.get(CONF_FALLBACK_REGULAR, True),
                    ): bool,
                    vol.Required(
                        CONF_FALLBACK_EMERGENCY,
                        default=current.get(CONF_FALLBACK_EMERGENCY, True),
                    ): bool,
                }
            ),
        )
    ...
    async def async_step_users(
        ...
        return self.async_show_menu(
            ...
            description_placeholders={
                ...
                "users": ", ".join(
                    (
                        f"{profile.get(CONF_USER_NAME) or user_id} ({user_id})"
    ...
    async def async_step_add_detected_user(
        self, user_input: dict[str, Any] | None = None
    ) -> config_entries.ConfigFlowResult:
        ...
        choices = {
            user_id: (
                f"{item.get('name') or item.get('username') or 'MAX пользователь'}"
                f" — {user_id}"
            )
        ...
        return self.async_show_form(
            step_id="add_detected_user",
            data_schema=vol.Schema({vol.Required("user_id"): vol.In(choices)}),
        )

    async def async_step_add_detected_user_details(
        self, user_input: dict[str, Any] | None = None
    ) -> config_entries.ConfigFlowResult:
        ...
        if user_input is not None:
            ...
            users[user_id] = {
                CONF_USER_NAME: str(user_input[CONF_USER_NAME]).strip(),
                CONF_USER_ENABLED: bool(user_input[CONF_USER_ENABLED]),
                CONF_USER_PERMISSIONS: permissions,
            }
            return self._save_options({CONF_USERS: users})

        return self.async_show_form(
            step_id="add_detected_user_details",
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_USER_NAME, default=default_name): str,
                    vol.Required(CONF_USER_ENABLED, default=True): bool,
                    vol.Required(
                        CONF_USER_PERMISSIONS, default=[]
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(CONF_USER_ENABLED, default=True): bool,
                    vol.Required(
                        CONF_USER_PERMISSIONS, default=[PERM_ALL]
                    ): PERMISSIONS_SELECTOR,
                }
            ),
            errors=errors,
    ...
    async def async_step_edit_user(
        ...
        if not users:
            return await self.async_step_users()
        choices = {
            user_id: f"{profile.get(CONF_USER_NAME) or user_id} — {user_id}"
            for user_id, profile in users.items()
        }
        if user_input is not None:
    ...
    async def async_step_edit_user_details(
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_USER_NAME, default=profile.get(CONF_USER_NAME, "")
                    ): str,
                    vol.Required(
                        CONF_USER_ENABLED,
                        default=profile.get(CONF_USER_ENABLED, True),
                    ): bool,
                    vol.Required(
                        CONF_USER_PERMISSIONS,
                        default=profile.get(CONF_USER_PERMISSIONS, []),
                    ): PERMISSIONS_SELECTOR,
                }
            ),
            description_placeholders={"user_id": user_id},
    ...
    async def async_step_delete_user_confirm(
        self, user_input: dict[str, Any] | None = None
    ) -> config_entries.ConfigFlowResult:
        users = self._users()
        user_id = self._selected_user_id
        ...
        if user_input is not None:
            ...
            return self._save_options({CONF_USERS: users})
        return self.async_show_form(
            step_id="delete_user_confirm",
            data_schema=vol.Schema({}),
            description_placeholders={
                "user_id": user_id,
                "name": profile.get(CONF_USER_NAME) or user_id,
            },
        )
