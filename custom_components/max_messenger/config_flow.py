...
from .api import (
    MaxMessengerApi,
    MaxMessengerApiError,
    MaxMessengerAuthError,
)
from .const import (
    CONF_ALLOWED_USERS,
...
PERMISSION_OPTIONS = [
    ...
    {"value": "braga", "label": "Контроллер"},
    {"value": "braga_emergency", "label": "Контроллер: аварийная остановка"},
...
def _normalize_users(value: Any) -> dict[str, dict[str, Any]]:
    ...
    for raw_id, raw_profile in value.items():
        ...
        clean_permissions = [
            ...
            if item == PERM_ALL or item in PERMISSIONS
        ]
...
class MaxMessengerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(
        ...
        return MaxMessengerOptionsFlow()
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(CONF_TARGET_TYPE, default=TARGET_USER): vol.In(
...
class MaxMessengerOptionsFlow(OptionsFlowWithReload):
    ...
    async def async_step_add_detected_user(
        ...
        choices = {
            user_id: (
                f"{item.get('name') or item.get('username') or 'MAX пользователь'} — {user_id}"
            )
    ...
    async def async_step_add_detected_user_details(
        ...
        default_name = (
            detected.get("name") or detected.get("username") or f"MAX {user_id}"
        )
        if user_input is not None:
            users = self._users()
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(CONF_USER_PERMISSIONS, default=[]): PERMISSIONS_SELECTOR,
                }
            ),
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Required(CONF_USER_ENABLED, default=True): bool,
                    vol.Required(CONF_USER_PERMISSIONS, default=[PERM_ALL]): PERMISSIONS_SELECTOR,
                }
            ),
            errors=errors,
        )
    ...
    async def async_step_edit_user_details(
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_USER_NAME, default=profile.get(CONF_USER_NAME, "")): str,
                    vol.Required(CONF_USER_ENABLED, default=profile.get(CONF_USER_ENABLED, True)): bool,
                    vol.Required(CONF_USER_PERMISSIONS, default=profile.get(CONF_USER_PERMISSIONS, [])): PERMISSIONS_SELECTOR,
                }
            ),
            description_placeholders={"user_id": user_id},
        )
